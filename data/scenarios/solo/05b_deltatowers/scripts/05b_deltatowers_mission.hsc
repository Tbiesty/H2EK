...
(script dormant bridges_start
	...
	(game_save)
	(if (< (ai_living_count bridge_enemies) 12)
...
;	(sleep 180)
	...
	(ai_place gondola_01_b_buggers (- 5 (ai_living_count gondola_01_bugs_new)))
...
(script dormant tower3_start
	...
	(if (< (ai_living_count all_allies) 3)
...
(script dormant sunken_chamber_start
	...
	(if (difficulty_legendary)
		(begin
			(set sunken_sniper_total 1)
			(set sunken_jacks_total 2)
...
;	(wake sunken_holo_translate)
	...
	;		(> (ai_combat_status sunken_chamber_enemies) ai_combat_status_idle)
			...
			(begin
				(if (< (ai_living_count sunken_chamber_enemies) 16)
					(ai_place sunk_chamber_bugs_L_02 2)
				)
				(sunken_try_js_R2)
			)
	)
	...
	;		(> (ai_combat_status sunken_chamber_enemies) ai_combat_status_idle)
			...
			(begin
				(if (< (ai_living_count sunken_chamber_enemies) 16)
					(ai_place sunk_chamber_bugs_R_02 2)
				)
				(sunken_try_js_L2)
			)
	)
	...
	(if 
		(AND
			...
			(begin
				(if (< (ai_living_count sunken_chamber_enemies) 16)
					(ai_place sunk_chamber_exit_j01_L)
				)
				(if (< (ai_living_count sunken_chamber_enemies) 16)
					(ai_place sunk_chamber_exit_e_L)
				)
			)
	)
	(if 
		(AND
			...
			(begin
				(if (< (ai_living_count sunken_chamber_enemies) 16)
					(ai_place sunk_chamber_exit_j01_R)
				)
				(if (< (ai_living_count sunken_chamber_enemies) 16)
					(ai_place sunk_chamber_exit_e_R)
				)
			)
	)
	(if
...
(script dormant sunken_tunnel2_start
	...
	(ai_place tunnel_02_elites_01 2)
	(ai_place tunnel_02_elites_02 2)
...
;	(ai_place tunnel_02_buggers_02 3)
	...
	(ai_place tunnel_02_elites_04 2)
...
(script command_script island_phantom_path
	...
	(if (< (ai_living_count island_all_enemies) 16)
		(ai_place island_phantom_elites 2)
	)
	(if (< (ai_living_count island_all_enemies) 16)
		(ai_place island_phantom_jackals 3)
	)
	(if (< (ai_living_count island_all_enemies) 16)
		(ai_place island_phantom_grunts 3)
	)
...
(script dormant island_exterior_start
	...
	(if (difficulty_legendary)
		(begin
			...
			(if (< (ai_living_count island_all_enemies) 16)
				(ai_place island_ext_grunts_01 3)
			)
			(if (< (ai_living_count island_all_enemies) 16)
				(ai_place island_ext_grunts_02 3)
			)
		)
	)
	(if (difficulty_heroic)
...
;9/11
	...
	(if (< (ai_living_count island_all_enemies) 16)
		(ai_place near_gondola_02_jackals)
	)
	(if (< (ai_living_count island_all_enemies) 16)
		(ai_place near_gondola_02_grunts)
	)
...
(script command_script boost_test
	...
	(cs_shoot TRUE)
	(cs_enable_moving TRUE)
	(sleep_until (< (objects_distance_to_object (players) (ai_get_object ai_current_actor)) 25))
)
...
;9/11
	...
	(sleep 300)
	(ai_place gondola_02_banshees 3)
